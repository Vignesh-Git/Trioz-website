...
const SectionSliderNewCategories: FC<SectionSliderNewCategoriesProps> = ({
  ...
  // initialize to itemPerRow so we have a sensible default before measurement
  const [numberOfItems, setNumberOfitem] = useState<number>(itemPerRow);

  const windowWidth = useWindowSize().width || 0;
  useEffect(() => {
  // compute desired items based on breakpoints, but always at least 1
  let computed: number = itemPerRow as number;
    if (windowWidth > 0 && windowWidth < 320) {
      computed = 1;
    } else if (windowWidth > 0 && windowWidth < 500) {
      computed = itemPerRow - 3;
    } else if (windowWidth > 0 && windowWidth < 1024) {
      computed = itemPerRow - 2;
    } else if (windowWidth > 0 && windowWidth < 1280) {
      computed = itemPerRow - 1;
    }

    setNumberOfitem(Math.max(1, computed));
  ...
  return (
    <div className={`nc-SectionSliderNewCategories ${className}`}>
      ...
      <MotionConfig
        transition={{
          x: { type: "spring", stiffness: 300, damping: 30, delay: 0.5 },
              ...
              <AnimatePresence initial={false} custom={direction}>
                {categories.map((item, indx) => (
                  <motion.li
                    ...
                    key={indx}
                    style={{
                      width: `calc(1/${numberOfItems || 1} * 100%)`,
                    }}
                  >
                    {renderCard(item)}
                  </motion.li>
